"""Calculadora auditable de Stuff para ratings-2.0, sin persistencia."""
...
@dataclass(frozen=True)
class PitcherStuffResult:
    mlb_id: int
    rating_model_version: str
    distribution_version: str
    rating: int | None
    weight_coverage: float
    components: list[StuffComponent] = field(default_factory=list)
    skipped_metrics: list[str] = field(default_factory=list)


def calculate_stuff_candidate(
    db: Session,
    *,
    mlb_id: int,
    season: int,
    ...
    pending: list[tuple] = []
    skipped: list[str] = []
    for metric, component_weight in STUFF_WEIGHTS.items():
        ...
        distribution = db.query(LeagueMetricDistribution).filter_by(
            season=season, role="PITCHER", metric=metric,
            pitch_type=None, pitch_family=None,
    ...
    sufficient = coverage + 1e-12 >= MIN_STUFF_WEIGHT_COVERAGE
    components = [
        StuffComponent(
            metric=metric, observed=observed, league_baseline=baseline,
            sample_size=sample_size, stabilization=stabilization,
            shrinkage_weight=shrinkage_weight, adjusted=adjusted,
            percentile=percentile, rating=component_rating,
    ...
    return PitcherStuffResult(
        ...
        components=components, skipped_metrics=skipped,
    )
