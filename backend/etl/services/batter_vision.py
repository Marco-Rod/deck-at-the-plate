"""Calculadora auditable de Vision para batter ratings-2.0, sin persistencia."""
...
@dataclass(frozen=True)
class BatterVisionRating:
    mlb_id: int
    rating_model_version: str
    distribution_version: str
    rating: int | None
    components: list[BatterVisionComponent] = field(default_factory=list)
    skipped_metrics: list[str] = field(default_factory=list)


def _observed_value(batter: BatterSeasonStats, metric: str):
    if metric == "walk_rate":
        return batter.walks / batter.pa if batter.pa else None
    if metric == "strikeout_rate":
        return batter.strikeouts / batter.pa if batter.pa else None
    return getattr(batter, metric)


def calculate_batter_vision(
    db: Session,
    *,
    mlb_id: int,
    season: int,
    ...
    components: list[BatterVisionComponent] = []
    skipped: list[str] = []
    for metric, component_weight in BATTER_VISION_WEIGHTS.items():
        ...
        stabilization = BATTER_VISION_STABILIZATIONS[metric]
